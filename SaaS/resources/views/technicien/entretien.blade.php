        ...
        @foreach ($fiches as $fiche)
            ...
            <details class="group bg-white shadow-md rounded-xl overflow-hidden transition-all duration-300 hover:shadow-lg">
                ...
                <div class="px-6 pb-4 pt-2 text-sm text-gray-700 bg-gray-50">
                    ...
                    <p><strong>Panne et/ou vigilance:</strong> {{ $fiche->ficheable->panne_vigilance }}</p>
                    @if($fiche->ficheable->derniere_date == null)
                        <p><strong>Date prévue:</strong> Pas encore planifiée</p>
                    @else
                        <p><strong>Prochaine date prévue:</strong> {{ \Carbon\Carbon::parse($fiche->ficheable->derniere_date)->format('d/m/Y') }}</p>
                    @endif
